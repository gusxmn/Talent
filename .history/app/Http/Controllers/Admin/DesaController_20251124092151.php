...
class DesaController extends Controller
{
    ...
    {
        try {
            // Get limit from query or default to 15000 (all data)
            $limit = request('limit', 15000);
            $limit = min((int)$limit, 15000); // Max 15000 records per request
            ...
            return response()->json([
                'success' => true,
                'count' => $desas->count(),
                'total' => Desa::where('status', true)->count(),
                'data' => $desas
            ]);
        } catch (\Exception $e) {
            return response()->json([
