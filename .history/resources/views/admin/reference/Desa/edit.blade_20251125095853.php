                    ...
                    <div style="max-height: 600px; overflow-y: auto;" id="desa-list-container">
                        @forelse($desas ?? [] as $des)
                            <div style="padding: 0.75rem 1rem; border-bottom: 1px solid #e9ecef; cursor: pointer; transition: background-color 0.2s; {{ $des->id == $desa->id ? 'background-color: #fff3cd;' : '' }}" 
                                 class="{{ $des->id == $desa->id ? 'active' : '' }}"
                                 onmouseover="if (this.className !== 'active') this.style.backgroundColor='#f8f9fa'" onmouseout="this.style.backgroundColor='{{ $des->id == $desa->id ? '#fff3cd' : 'white' }}'">
