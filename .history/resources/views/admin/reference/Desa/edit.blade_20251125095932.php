        ...
        <div class="col-lg-6">
            <div class="card">
                ...
                <div class="card-body p-0">
                    <div style="max-height: 600px; overflow-y: auto;" id="desa-list-container">
                        @forelse($desas ?? [] as $des)
                            <div style="padding: 0.75rem 1rem; border-bottom: 1px solid #e9ecef; cursor: pointer; transition: background-color 0.2s;@if($des->id == $desa->id) background-color: #fff3cd;@endif" 
                                 class="desa-item @if($des->id == $desa->id)active@endif"
                                 onmouseover="this.style.backgroundColor = this.classList.contains('active') ? '#fff3cd' : '#f8f9fa'" 
                                 onmouseout="this.style.backgroundColor = this.classList.contains('active') ? '#fff3cd' : 'white'">
...
<script>
    ...
    document.getElementById('kecamatan_id')?.addEventListener('change', async function() {
        ...
        try {
            ...
            let html = '';
            data.forEach(item => {
                const isActive = item.id === currentDesaId;
                ...
                html += `
                    <div style="padding: 0.75rem 1rem; border-bottom: 1px solid #e9ecef; cursor: pointer; transition: background-color 0.2s; background-color: ${bgColor};" 
                        ...
                        <div class="d-flex justify-content-between align-items-start">
                            <div>
                                <div class="fw-bold">${item.nama_desa}</div>
                                <small class="text-muted">Kode: <code>${item.kode_desa}</code></small>
                            </div>
