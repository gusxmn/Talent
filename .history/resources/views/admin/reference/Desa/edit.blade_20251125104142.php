    ...
    <div class="p-4 rounded shadow-sm" style="background-color: #cccccc;">
        ...
        <form action="{{ route('admin.reference.desa.update', $desa->id) }}" method="POST">
            ...
            <div class="mb-3 p-3 border rounded" style="background-color: #e0e0e0;">
                    ...
                    <div class="col-md-3">
                        ...
                        <select id="provinsi" class="form-select form-select-sm" required>
                            ...
                            @foreach ($provinces as $province)
                                <option value="{{ $province->id }}" {{ old('province_id', $desa->kecamatan->kabupaten->province_id ?? '') == $province->id ? 'selected' : '' }}>
                                    {{ $province->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <input type="hidden" name="province_id" id="province_id" value="{{ old('province_id', $desa->kecamatan->kabupaten->province_id ?? '') }}">
                <input type="hidden" name="kabupaten_id" id="regency_id" value="{{ old('kabupaten_id', $desa->kecamatan->kabupaten_id) }}">
                <input type="hidden" name="kecamatan_id" id="district_id" value="{{ old('kecamatan_id', $desa->kecamatan_id) }}">
                <input type="hidden" name="desa_id" id="village_id" value="{{ old('desa_id', $desa->id) }}">
            </div>
            ...
            <div class="mb-3">
                <label for="description" class="font-weight: normal;">Deskripsi</label>
                <textarea class="form-control @error('description') is-invalid @enderror" 
                          id="description" name="description" rows="3" 
                          placeholder="Deskripsi tambahan tentang desa/kelurahan">{{ old('description', $desa->description ?? '') }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="d-flex gap-2 justify-content-end">
                <a href="{{ route('admin.reference.desa.index') }}" class="btn btn-secondary">
