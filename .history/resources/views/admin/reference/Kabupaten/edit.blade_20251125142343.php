...
<script>
    const initialValues = {
        provinsiId: '{{ old("provinsi_id", $regency->provinsi_id ?? "") }}',
        namakabupaten: '{{ old("nama_kabupaten", $regency->nama_kabupaten ?? "") }}',
        status: '{{ old("status", $regency->status) }}'
    };

    function resetForm() {
        document.getElementById('provinsi').value = initialValues.provinsiId;
        document.getElementById('nama_kabupaten').value = initialValues.namakabupaten;
