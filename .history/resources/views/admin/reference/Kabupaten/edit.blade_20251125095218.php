                ...
                <div class="card-body p-0">
                    <div style="max-height: 600px; overflow-y: auto;">
                        @forelse($regencies ?? [] as $reg)
                            <div style="padding: 0.75rem 1rem; border-bottom: 1px solid #e9ecef; cursor: pointer; transition: background-color 0.2s; {{ $reg->id == $regency->id ? 'background-color: #fff3cd;' : '' }}" 
                                 onmouseover="this.style.backgroundColor='#f8f9fa'" onmouseout="this.style.backgroundColor='{{ $reg->id == $regency->id ? '#fff3cd' : 'white' }}'">
                                <div class="d-flex justify-content-between align-items-start">
                                    <div>
                                        <div class="fw-bold">{{ $reg->name }}</div>
                                        <small class="text-muted">Kode: <code>{{ $reg->id }}</code></small>
                                    </div>
                                    <span class="badge {{ $reg->status == 1 ? 'bg-success' : 'bg-secondary' }}">
                                        {{ $reg->status == 1 ? 'Aktif' : 'Nonaktif' }}
                                    </span>
                                </div>
                            </div>
                        @empty
