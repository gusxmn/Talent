    ...
    <div class="p-4 rounded shadow-sm" style="background-color: #cccccc;">
        ...
        <form action="{{ route('admin.reference.kecamatan.update', $kecamatan->id) }}" method="POST">
            ...
            <div class="mb-3 p-3 border rounded" style="background-color: #e0e0e0;">
                    ...
                    <div class="col-md-3">
                        ...
                        <select id="provinsi" class="form-select form-select-sm" required>
                            ...
                            @foreach ($provinces as $province)
                                <option value="{{ $province->id }}" {{ old('province_id', $kecamatan->kabupaten->province_id ?? '') == $province->id ? 'selected' : '' }}>
                                    {{ $province->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <input type="hidden" name="province_id" id="province_id" value="{{ old('province_id', $kecamatan->kabupaten->province_id ?? '') }}">
                <input type="hidden" name="kabupaten_id" id="regency_id" value="{{ old('kabupaten_id', $kecamatan->kabupaten_id) }}">
                <input type="hidden" name="kecamatan_id" id="district_id" value="{{ old('kecamatan_id', $kecamatan->id) }}">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" 
                          ...
                          placeholder="Deskripsi tambahan tentang kecamatan">{{ old('description', $kecamatan->description ?? '') }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="d-flex gap-2 justify-content-end">
                <a href="{{ route('admin.reference.kecamatan.index') }}" class="btn btn-secondary">
