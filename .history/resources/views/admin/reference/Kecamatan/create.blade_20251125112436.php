...
<div class="form-isian-area p-4" style="background-color: #e9ecef;">
    ...
    <div class="p-4 rounded shadow-sm" style="background-color: #cccccc;">
        ...
        <form action="{{ route('admin.reference.kecamatan.store') }}" method="POST" data-kabupatens="{{ json_encode($kabupatensGrouped) }}">
            @csrf
...
<script>
    ...
    // Data kabupatens grouped by province (passed from controller)
    const kabupatensData = @json($kabupatensGrouped);
    ...
    window.addEventListener('DOMContentLoaded', function() {
        ...
        // Restore old kabupaten value if form has errors
        const oldKabupatenId = '{{ old("kabupaten_id") }}';
        
        if (oldKabupatenId) {
            // Find the province for this kabupaten
            ...
            Object.keys(kabupatensData).forEach(provinceId => {
                ...
                const found = kabupatensForProvince.find(k => k.id === oldKabupatenId);
                if (found) {
            ...
            if (foundProvinceId) {
                provinsiEl.value = foundProvinceId;
