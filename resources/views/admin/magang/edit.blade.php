...
<div class="judul-form-area text-white p-3" style="background-color: #28a745;">
    <h4 class="mb-0 fw-bold">Edit Lowongan Magang</h4>
</div>
    ...
    <div class="p-4 rounded shadow-sm" style="background-color: #cccccc;">
        <form action="{{ route('admin.magang.update', $magang->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3 p-3 border rounded" style="background-color: #e0e0e0;">
                        ...
                        <select id="provinsi" class="form-select form-select-sm" required>
                            ...
                            @foreach ($provinces as $prov)
                                <option value="{{ $prov->id }}" 
                                    {{ $magang->provinsi_id == $prov->id ? 'selected' : '' }}>
                                    {{ $prov->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <input type="hidden" name="provinsi_id" id="province_id" value="{{ old('provinsi_id', $magang->provinsi_id) }}">
                <input type="hidden" name="kabupaten_id" id="regency_id" value="{{ old('kabupaten_id', $magang->kabupaten_id) }}">
                <input type="hidden" name="kecamatan_id" id="district_id" value="{{ old('kecamatan_id', $magang->kecamatan_id) }}">
                <input type="hidden" name="desa_id" id="village_id" value="{{ old('desa_id', $magang->desa_id) }}">
            ...
            <div class="d-flex justify-content-end pt-3">
                ...
                <button type="submit" class="btn text-white" style="background-color: #0a4988; border-radius: 20px; min-width: 100px;">
                    Simpan
                </button>
            </div>
        </form>
...
<script>
    ...
    provinsiEl.addEventListener('change', function () {
        ...
        if (provinceId) {
            loadWilayah(`/admin/get-regencies/${provinceId}`, kabupatenEl, 'Kabupaten/Kota');
        }
    ...
    kabupatenEl.addEventListener('change', function () {
        ...
        if (regencyId) {
            loadWilayah(`/admin/get-districts/${regencyId}`, kecamatanEl, 'Kecamatan');
        }
        updateLocationDisplay();
    });
    ...
    kecamatanEl.addEventListener('change', function () {
        ...
        if (districtId) {
            loadWilayah(`/admin/get-villages/${districtId}`, desaEl, 'Desa/Kelurahan');
        }
        updateLocationDisplay();
    });
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Aktifkan dropdown yang sudah ada pilihan
        if (provinsiEl.value) {
